...
hooksCommand
  ...
  .action(async (options) => {
    ...
    if (hookType !== 'pre-push' && hookType !== 'pre-commit') {
      console.error('❌ Invalid hook type. Use "pre-push" or "pre-commit"')
      process.exit(1)
    }
...
hooksCommand
  ...
  .action(async (options) => {
    ...
    console.log(`🗑️ Uninstalling ${hookType} hook...`)
...
hooksCommand
  ...
  .action(async () => {
    ...
    const prePushIcon = status.prePush ? '✅' : '❌'
    const preCommitIcon = status.preCommit ? '✅' : '❌'
    ...
    if (!status.prePush && !status.preCommit) {
      console.log('\n💡 Run "envvault hooks install" to install hooks')
    }
  })
